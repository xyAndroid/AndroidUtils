...
import com.xy.lib.callback.XActivityLifecycleCallbacks;
 ...
 */
public class XActivityLifecycleUtils implements Application.ActivityLifecycleCallbacks {
    private boolean isRegister;

    public static XActivityLifecycleUtils getInstance() {
        return BLActivityLifecycleUtilsInner.activityLifecycleUtils;
    }

    private static class BLActivityLifecycleUtilsInner {
        static XActivityLifecycleUtils activityLifecycleUtils = new XActivityLifecycleUtils();
    }


    private final LinkedList<Activity> mActivityList = new LinkedList<>();

    private final List<OnXAppStatusChangedListener> mStatusListeners = new ArrayList<>();
    private final Map<Activity, List<XActivityLifecycleCallbacks>> mActivityLifecycleCallbacksMap = new ConcurrentHashMap<>();
    ...
    public void unregisterActivityLifecycleCallbacks(Application app) {
        isRegister = false;
        mActivityList.clear();
    ...
    private void checkRegister(){
        if (!isRegister){
            throw new UnsupportedOperationException("please call BLActivityLifecycleUtils.registerActivityLifecycleCallbacks(Application)");
        }
    }
    ...
    Activity getTopActivity() {
        ...
        for (Activity activity : activityList) {
            if (!XActivityUtils.isActivityAlive(activity)) {
                continue;
            }
            return activity;
        }
    ...
    void addOnAppStatusChangedListener(final OnXAppStatusChangedListener listener) {
        mStatusListeners.add(listener);
    }

    void removeOnAppStatusChangedListener(final OnXAppStatusChangedListener listener) {
        mStatusListeners.remove(listener);
    }

    void addActivityLifecycleCallbacks(final Activity activity,
                                       final XActivityLifecycleCallbacks listener) {
    ...
    private void addActivityLifecycleCallbacksInner(final Activity activity,
                                                    final XActivityLifecycleCallbacks lifecycleCallbacks) {
        checkRegister();
        List<XActivityLifecycleCallbacks> callbacks = mActivityLifecycleCallbacksMap.get(activity);
        if (callbacks == null) {
    ...
    void removeActivityLifecycleCallbacks(final Activity activity,
                                          final XActivityLifecycleCallbacks callbacks) {
    ...
    private void consumeActivityLifecycleCallbacks(Activity activity, Lifecycle.Event event) {
        List<XActivityLifecycleCallbacks> listeners = mActivityLifecycleCallbacksMap.get(activity);
        if (listeners != null) {
            for (XActivityLifecycleCallbacks listener : listeners) {
    ...
    private void processHideSoftInputOnActivityDestroy(final Activity activity, boolean isSave) {
        if (isSave) {
            ...
            XThreadUtils.runOnUiThreadDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    private void postStatus(final Activity activity, final boolean isForeground) {
        ...
        for (OnXAppStatusChangedListener statusListener : mStatusListeners) {
            if (isForeground) {
    ...
    private List<Activity> getActivitiesByReflect() {
        ...
        try {
            ...
            XLog.e("UtilsActivityLifecycle", "getActivitiesByReflect: " + e.getMessage());
        }
        if (topActivity != null) {
    ...
    private Object getActivityThreadInActivityThreadStaticField() {
        try {
            ...
            XLog.e("UtilsActivityLifecycle", "getActivityThreadInActivityThreadStaticField: " + e.getMessage());
            return null;
        }
    }
    ...
    private Object getActivityThreadInActivityThreadStaticMethod() {
        try {
            ...
            XLog.e("UtilsActivityLifecycle", "getActivityThreadInActivityThreadStaticMethod: " + e.getMessage());
            return null;
        }
    }
    ...
    private Object getActivityThreadInLoadedApkField() {
        try {
            ...
            Object mLoadedApk = mLoadedApkField.get(XAndroidUtils.app());
            ...
            XLog.e("UtilsActivityLifecycle", "getActivityThreadInLoadedApkField: " + e.getMessage());
            return null;
        }
    }
    ...
    private static void setAnimatorsEnabled() {
        ...
        try {
            ...
            if (sDurationScale == 0f) {
                ...
                XLog.i("UtilsActivityLifecycle", "setAnimatorsEnabled: Animators are enabled now!");
            }
